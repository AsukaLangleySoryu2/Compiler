...
public class Parser {
    ...
    private LexAnalyse lexAnalyse;// 锟绞凤拷锟斤拷锟斤拷锟斤拷
    ArrayList<Word> wordList = new ArrayList<Word>();// 锟斤拷锟绞憋拷
    Stack<AnalyseNode> analyseStack = new Stack<AnalyseNode>();// 锟斤拷锟斤拷栈
    Stack<String> semanticStack = new Stack<String>();// 锟斤拷锟斤拷栈
    ArrayList<FourElement> fourElemList = new ArrayList<FourElement>();// 锟斤拷元式锟叫憋拷
    ArrayList<Error> errorList = new ArrayList<Error>();// 锟斤拷锟斤拷锟斤拷息锟叫憋拷
    StringBuffer bf;// 锟斤拷锟斤拷栈锟斤拷锟斤拷锟斤拷
    int errorCount = 0;// 统锟狡达拷锟斤拷锟斤拷锟�
    boolean graErrorFlag = false;// 锟斤法锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷志
    int tempCount = 0;// 锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷时锟斤拷锟斤拷
    int fourElemCount = 0;// 统锟斤拷锟斤拷元式锟斤拷锟斤拷
    ...
    AnalyseNode top;// 锟斤拷前栈锟斤拷元锟斤拷
    Word firstWord;// 锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷
    String OP = null;
    String ARG1, ARG2, RES;
    Error error;
    // int if_fj,if_rj,while_fj,while_rj,for_fj,for_rj;
    Stack<Integer> if_fj, if_rj, while_fj, while_rj, for_fj, for_rj;// if while for 锟斤拷转锟斤拷址栈
    ...
    public void grammerAnalyse() {// LL1锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤法锟斤拷锟斤拷
        if (lexAnalyse.isFail())
            javax.swing.JOptionPane.showMessageDialog(null, "锟绞凤拷锟斤拷锟斤拷未通锟斤拷锟斤拷锟斤拷锟杰斤拷锟斤拷锟斤法锟斤拷锟斤拷");
        bf = new StringBuffer();
        int gcount = 0;
        error = null;
        ...
        while (!analyseStack.empty() && !wordList.isEmpty()) {
            ...
            bf.append('\n');
            bf.append("锟斤拷锟斤拷" + gcount + "\t");
            if (gcount++ > 10000) {
                graErrorFlag = true;
                break;
            }
            top = analyseStack.get(0);// 锟斤拷前栈锟斤拷元锟斤拷
            firstWord = wordList.get(0);// 锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷
            if (firstWord.value.equals("#")// 锟斤拷锟斤拷锟斤拷锟斤拷
                    && top.name.equals("#")) {
                bf.append("\n");
            ...
            } else if (AnalyseNode.isTerm(top)) {// 锟秸硷拷锟斤拷时锟侥达拷锟斤拷
                termOP(top.name);
            } else if (AnalyseNode.isNonterm(top)) {
                nonTermOP(top.name);
            } else if (top.type.equals(AnalyseNode.ACTIONSIGN)) {// 栈锟斤拷锟角讹拷锟斤拷锟斤拷锟斤拷时锟侥达拷锟斤拷
                actionSignOP();
            }

            bf.append("锟斤拷前锟斤拷锟斤拷栈:");
            for (int i = 0; i < analyseStack.size(); i++) {
                bf.append(analyseStack.get(i).name);
                // System.out.println(analyseStack.get(i).name);
            }
            bf.append("\t").append("锟斤拷锟斤拷锟斤拷锟脚达拷锟斤拷");
            for (int j = 0; j < wordList.size(); j++) {
                bf.append(wordList.get(j).value);
            }
            bf.append("\t").append("锟斤拷锟斤拷栈:");
    ...
    private void termOP(String term) {
        if (firstWord.type.equals(Word.INT_CONST) || firstWord.type.equals(Word.CHAR_CONST) ||
            ...
            analyseStack.remove(0);
            wordList.remove(0);
            error = new Error(errorCount, "锟斤法锟斤拷锟斤拷", firstWord.line, firstWord);
            errorList.add(error);
            graErrorFlag = true;
        }
    ...
    private void nonTermOP(String nonTerm) {
        ...
        if (nonTerm.equals("T'"))
            nonTerm = "6";
        switch (nonTerm.charAt(0)) {// 栈锟斤拷为锟斤拷锟秸斤拷锟斤拷锟斤拷锟�
            // N:S,B,A,C,,X,R,Z,Z锟斤拷,U,U锟斤拷,E,E锟斤拷,H,H锟斤拷,G,M,D,L,L锟斤拷,T,T锟斤拷,F,O,P,Q
            case 'S':
                if (firstWord.value.equals("void")) {
                    ...
                    analyseStack.remove(0);
                    wordList.remove(0);
                    error = new Error(errorCount, "锟斤拷锟斤拷锟斤拷没锟叫凤拷锟斤拷值", firstWord.line, firstWord);
                    errorList.add(error);
                    graErrorFlag = true;
                }
                break;
            ...
            case 'Y':
                if (firstWord.value.equals("int")) {
                    ...
                    analyseStack.remove(0);
                    wordList.remove(0);
                    error = new Error(errorCount, "锟角凤拷锟斤拷锟斤拷锟斤拷锟斤拷", firstWord.line, firstWord);
                    errorList.add(error);
                    graErrorFlag = true;
                }
                break;
            case 'Z':
                if (firstWord.type.equals(Word.IDENTIFIER)) {
                    ...
                    analyseStack.remove(0);
                    wordList.remove(0);
                    error = new Error(errorCount, "锟角凤拷锟斤拷识锟斤拷", firstWord.line, firstWord);
                    errorList.add(error);
                    graErrorFlag = true;
                }
                break;
            ...
            case 'P':
                if (firstWord.type.equals(Word.IDENTIFIER)) {
                    ...
                    analyseStack.remove(0);
                    wordList.remove(0);
                    error = new Error(errorCount, "锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟�", firstWord.line, firstWord);
                    errorList.add(error);
                    graErrorFlag = true;
                }
                break;
            case 'E':
                if (firstWord.type.equals(Word.IDENTIFIER)) {
                    ...
                    analyseStack.remove(0);
                    wordList.remove(0);
                    error = new Error(errorCount, "锟斤拷锟杰斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟�", firstWord.line, firstWord);
                    errorList.add(error);
                    graErrorFlag = true;
                }
                break;
            ...
            case 'D':
                if (firstWord.value.equals("==")) {
                    ...
                    analyseStack.remove(0);
                    wordList.remove(0);
                    error = new Error(errorCount, "锟角凤拷锟斤拷锟斤拷锟�", firstWord.line, firstWord);
                    errorList.add(error);
                    graErrorFlag = true;
                }
                break;
            case 'G':
                if (firstWord.type.equals(Word.IDENTIFIER)) {
                    ...
                    analyseStack.remove(0);
                    wordList.remove(0);
                    error = new Error(errorCount, "锟斤拷锟杰斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷突锟斤拷锟斤拷挪锟狡ワ拷锟�", firstWord.line, firstWord);
                    errorList.add(error);
                    graErrorFlag = true;
                }
                break;
            ...
            case 'F':
                if (firstWord.type.equals(Word.IDENTIFIER)) {
                    ...
                    // wordList.remove(0);
                    // error=new
                    // Error(errorCount,"锟斤拷锟杰斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟�",firstWord.line,firstWord);
                    // errorList.add(error);
                    // graErrorFlag=true;
                }
                break;
    ...
    public String outputLL1() throws IOException {
        ...
        if (!file.exists()) {
            file.mkdirs();
            file.createNewFile();// 锟斤拷锟斤拷锟斤拷锟侥硷拷锟斤拷锟斤拷锟节就达拷锟斤拷锟斤拷
        }
        ...
        if (graErrorFlag) {
            Error error;
            pw1.println("锟斤拷锟斤拷锟斤拷息锟斤拷锟铰ｏ拷");

            pw1.println("锟斤拷锟斤拷锟斤拷锟絓t锟斤拷锟斤拷锟斤拷息\t锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷 \t锟斤拷锟襟单达拷");
            for (int i = 0; i < errorList.size(); i++) {
                error = errorList.get(i);
                pw1.println(error.id + "\t" + error.info + "\t\t" + error.line + "\t" + error.word.value);
            }
        } else {
            pw1.println("锟斤法锟斤拷锟斤拷通锟斤拷锟斤拷");
        }
        pw1.close();
        return path + "/LL1.txt";
    }
    ...
    public String outputFourElem() throws IOException {
        ...
        OutputStreamWriter osw1 = new OutputStreamWriter(bos, "utf-8");
        PrintWriter pw1 = new PrintWriter(osw1);
        pw1.println("锟斤拷锟缴碉拷锟斤拷元式锟斤拷锟斤拷");
        pw1.println("锟斤拷牛锟絆P,ARG1锟斤拷ARG2锟斤拷RESULT锟斤拷");
        FourElement temp;
